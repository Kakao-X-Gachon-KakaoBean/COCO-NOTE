describe('작업관리 페이지 테스트', () => {
  ...
  context('하위 작업 세부 페이지에 들어온 경우', () => {
    ...
    it('하위 작업 세부 페이지에 존재 하는 문구들이 있어야 한다.', () => {
      cy.get('.css-gmd4cy').should('have.text', '회원가입');
      cy.get('.css-7vi76i').should('be.visible');
      cy.get('.css-19w7i6i > .ant-select > .ant-select-selector').should('have.text', '임인범');
      cy.get('[style="width: 7vw;"] > .ant-select-selector').should('have.text', '진행 중');
      cy.get('.css-siynck').should('have.text', '카카오, 구글 등 소셜 로그인을 포함한다.');
  ...
  context('삭제하기 버튼을 누르는 경우', () => {
    ...
    it('삭제에 실패한 경우 실패 메시지가 상단에 보여야 한다.', () => {
      cy.get('.ant-modal-footer > .ant-btn-primary').should('have.text', 'OK').click();
      cy.get('.Toastify__toast-body > :nth-child(2)').should('have.text', '하위작업 삭제에 실패하였습니다.');
    });
  });
